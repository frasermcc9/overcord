...
function channelFilterExact(search: string) {
  return (chan: GuildChannel | ThreadChannel) => chan.type === "GUILD_TEXT" && chan.name.toLowerCase() === search;
}

function channelFilterInexact(search: string) {
  return (chan: GuildChannel | ThreadChannel) => chan.type === "GUILD_TEXT" && chan.name.toLowerCase().includes(search);
}
