...
export default abstract class NextCommand {
  ...
  protected commandDidInhibit(sourceMessage: Message, issue: string): any {
    sourceMessage.channel.send(`This command has a cooldown. The specific problem is as follows: ${codify(issue)}}`);
    Log.warn(issue);
  }
  ...
  readonly handleExecutionPolicy = async (args: CommandHandlerArgs) => {
    ...
    const commandShouldBlock = ({ message, client, permissionManager }: CommandHandlerArgs): ExecutionPolicyResult => {
